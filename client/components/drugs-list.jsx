...
import { selectSearchQuery } from "@/lib/store/searchSlice"
import { DrugCard } from "./drug-card"
...
export function DrugsList({ drugs }) {
  ...
  const handleShowLess = () => {
    setItemsToShow(8)
    // Scroll back to the top of the list
    window.scrollTo({ top: document.getElementById("drugs-list-top")?.offsetTop || 0, behavior: "smooth" })
  }
      ...
      {filteredDrugs.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-8 text-center">
            ...
            ) : filteredDrugs.length > 8 ? (
              <Button onClick={handleShowLess} variant="outline" className="px-8">
                <ChevronUp className="h-4 w-4 mr-2" />
                Show Less
              </Button>
            ) : null}
